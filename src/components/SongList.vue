<template>
  <div class="songs-list-main">
    <div class="list-header flex">
      ...
      <div v-if="typeSize !== 'mini'" class="columnAlbum">专辑</div>
      <div class="columnTime">时长</div>
    </div>
    <div ref="curSongRef" class="list-scroll" :style="{ height: height + 'px' }">
      <div class="list-main" :style="curSongSty">
        <div v-for="(item, index) in list" :key="item.id" :class="isCurSong(item, index)">
  ...
  export default {
    ...
    setup(props) {
      ...
      const curSongInfo = computed(() => {
        return playList.value[songStore.playIndex];
      });
      // 列表样式
      ...
      const addSongList = item => {
        songStore.setAddList({ list: [item] });
