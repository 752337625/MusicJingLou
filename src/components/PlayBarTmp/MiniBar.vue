    ...
    <div class="wrapper" :class="{ active: isMini }">
      ...
      <div class="play-main">
        ...
        <div class="audioProgress">
          <progress-line :progressWidth="audioProgressWidth" :isShowBtn="false" @set-progress-line="setAudioProgress" />
        </div>
  ...
  export default {
    ...
    setup(_props, { emit }) {
      ...
      const audioProgressWidth = computed(() => {
        // 音频进度条长度
        return (info['currentTime'] / config.globalProperties.$utils.formatSongSecond(curSongInfo.value.duration)) * 100 + '%';
      });
      // 点击拖拽进度条，设置当前时间
